...
- Depois de confirmado, a sugestão de "marcar saída" (seção 14.1) abrange todos os membros do grupo, já que o fechamento pela aba "Geral" é sempre do grupo como um todo — não só o credor que zerou o saldo ao ceder o crédito.
...
Sistema sugere marcar João e Maria como saída (fechamento é do grupo todo, via aba "Geral"),
mesmo João ainda devendo.
```
...
- Depois de registrar o pagamento, um modal de confirmação pergunta se deve marcar como "saiu do estabelecimento" — ação opcional, o dono pode recusar. Na aba de um **membro**, sugere só aquele cliente. Na aba **"Geral"**, o fechamento é do grupo como um todo, então sugere marcar a saída de **todos** os membros do grupo de uma vez, mesmo que algum tenha ficado com saldo residual (pagamento parcial) e não tenha zerado o saldo individualmente. O histórico de lançamentos nunca é excluído: mesmo com o saldo zerado, os lançamentos financeiros permanecem registrados para consulta no extrato.
